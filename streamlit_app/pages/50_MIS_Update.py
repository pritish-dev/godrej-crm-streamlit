...
if _so_qty_col:
    ...
    so_qty_num = pd.Series([float("nan")] * len(df), index=df.index)

if _wh_col:
    wh_col = df[_wh_col].astype(str).str.strip()
else:
    wh_col = pd.Series([""] * len(df), index=df.index)

neg_mask       = so_qty_num < 0
zbf11u_mask    = neg_mask & (wh_col == "ZBF11U")
zbf11t_mask    = neg_mask & (wh_col == "ZBF11T")
credited_qty   = int(so_qty_num[zbf11u_mask].sum()) if zbf11u_mask.any() else 0
to_be_credited = int(so_qty_num[zbf11t_mask].sum()) if zbf11t_mask.any() else 0

if _net_col:
    net_basic_num = pd.to_numeric(
        df[_net_col].astype(str).str.strip().str.replace(",", "", regex=False),
        errors="coerce"
    ).fillna(0)
else:
...
st.dataframe(
    ...
    column_config={
        ...
        "Sales Order Warehouse"     : st.column_config.TextColumn("Warehouse", width="small"),
        "Total Discount"            : st.column_config.NumberColumn("Total Discount", width="small"),
        "Total Net Basic"           : st.column_config.NumberColumn("Total Net Basic", width="small"),
        ...
        "Contact No"                : st.column_config.TextColumn("Contact No", width="small"),
        "Inventory Commitment Date" : st.column_config.TextColumn("Inv. Commitment Date", width="small"),
