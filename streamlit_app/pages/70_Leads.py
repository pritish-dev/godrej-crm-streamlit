...
with st.sidebar:
    ...
    quick_whatsapp = st.text_input("WhatsApp Number (Optional)", key="quick_whatsapp")
    ...
    if st.button("➕ Add Lead", key="quick_add"):
        if quick_name.strip():
            lead_data = {
                ...
                "email": quick_email,
                "phone": quick_phone,
                "whatsapp_number": quick_whatsapp,
                "alternate_number": quick_alternate,
                "address": quick_address,
                "assigned_to": quick_assigned if quick_assigned != "Unassigned" else "",
...
with st.expander("➕ Add New Lead - Detailed Entry", expanded=False):
    ...
    with tab5:
        ...
        with col1:
            ...
            whatsapp = st.text_input("WhatsApp Number (Optional)", key="manual_whatsapp")
            alternate = st.text_input("Alternate Number (Optional)", key="manual_alternate")
