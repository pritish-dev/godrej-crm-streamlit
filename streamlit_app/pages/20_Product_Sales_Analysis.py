...
# Handling the merged header found in your sheet
crm['CUSTOMER_NAME'] = get_cleaned_list(crm, "CUSTOMER NAMECONTACT NUMBER", "GUEST")

# --- MODULAR PRODUCT UNIFICATION (KRE PREFIX) ---
...
# SEARCH BAR
search_query = st.text_input("🔍 Search for a specific product name...", "").upper()

# ---------- 3. CALCULATION ----------
mask = (crm["YEAR"] == selected_year) & (crm["WORK_CAT"] == selected_cat)
analysis_df = crm[mask].copy()
...
summary = get_summary_df(analysis_df)
...
if search_query:
    summary = summary[summary["PRODUCT NAME"].str.contains(search_query, na=False)]
...
if not summary.empty:
    ...
    st.download_button("📥 Download Report (CSV)", csv_data, f"Sales_{selected_cat}.csv", "text/csv")
    ...
    st.markdown("""
        <style>
            ...
            .prod-table td { padding: 6px 15px; border: 1px solid #ccc; text-align: left; font-weight: bold; color: black; white-space: nowrap; }
        </style>
    """, unsafe_allow_html=True)

    # ---------- 6. RENDER TABLE & CUSTOMER LIST ----------
    ...
    st.subheader("👥 View Customers by Product")
    list_prods = sorted(summary["PRODUCT NAME"].unique())
    chosen_prod = st.selectbox("Select a product to see who bought it:", options=["-- Select --"] + list_prods)
    
    if chosen_prod != "-- Select --":
        cust_list = analysis_df[analysis_df["WORK_PROD"] == chosen_prod][["CUSTOMER_NAME", "WORK_DATE"]].drop_duplicates()
        cust_list.columns = ["CUSTOMER NAME", "PURCHASE DATE"]
    ...
    st.info(f"No records found matching your search or filters.")