...
# Loaded at module level using the same pattern as SENDER_EMAIL above.
# Set GOOGLE_DRIVE_INVOICES_FOLDER_ID in secrets.toml / .env / environment.
GOOGLE_DRIVE_INVOICES_FOLDER_ID = None

try:
    _fid = os.getenv("GOOGLE_DRIVE_INVOICES_FOLDER_ID", "").strip()
    if _fid and _fid != "PASTE_YOUR_FOLDER_ID_HERE":
        GOOGLE_DRIVE_INVOICES_FOLDER_ID = _fid
except Exception:
    pass

if GOOGLE_DRIVE_INVOICES_FOLDER_ID is None:
    try:
        import streamlit as st
        try:
            GOOGLE_DRIVE_INVOICES_FOLDER_ID = st.secrets["GOOGLE_DRIVE_INVOICES_FOLDER_ID"]
        except Exception:
            pass
    except Exception:
        pass
...
def fetch_invoice_from_drive(
    ...
    root_id = GOOGLE_DRIVE_INVOICES_FOLDER_ID
    if not root_id or root_id == "PASTE_YOUR_FOLDER_ID_HERE":
        return [], (
