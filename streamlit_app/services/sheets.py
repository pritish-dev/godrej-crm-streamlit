...
def get_df(sheet_name: str) -> pd.DataFrame:
    ...
    try:
        ...
        if sheet_name == "Users":
            ws.append_row(["username", "passwordhash", "fullname", "role", "active"])
        return pd.DataFrame()
...
def get_users_df() -> pd.DataFrame:
    ...
    if df is None or df.empty:
        ...
        return pd.DataFrame(columns=["username", "passwordhash", "fullname", "role", "active"])

    # Normalize headers and key columns
    df.columns = [str(c).strip().lower() for c in df.columns]
    for col in ["username", "passwordhash", "fullname", "role", "active"]:
...
def upsert_user(username: str, passwordhash: str, fullname: str, role: str, active: str = "Y"):
    """Create/update a user; writes canonical headers and trims values."""
    ensure_users_header()
    ws = sh.worksheet("Users")
    headers = ws.row_values(1)
    # Canonicalize headers if someone edited them:
    if [h.strip().lower() for h in headers] != ["username","passwordhash","fullname","role","active"]:
        ws.clear()
        ws.append_row(["username", "passwordhash", "fullname", "role", "active"])
        headers = ws.row_values(1)
