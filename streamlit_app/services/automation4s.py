...
def create_whatsapp_tabular_list(df_group, alert_type="delivery"):
    ...
    for _, row in df_group.iterrows():
        dd = row["CUSTOMER DELIVERY DATE"].strftime('%d-%b')
...
def get_alerts(df, team_df, alert_type="delivery"):
    ...
    if alert_type == "delivery":
        ...
        df["PENDING AMOUNT"] = pd.to_numeric(df["ORDER AMOUNT"], errors='coerce').fillna(0) - \
                               pd.to_numeric(df["ADVANCE RECEIVED"], errors='coerce').fillna(0)
        mask = (df["PENDING AMOUNT"] > 0)

    # STRICT FILTER: Only include records where Delivery Date is exactly Tomorrow (1 day away)
    filtered_df = df[mask & (df["CUSTOMER DELIVERY DATE"].dt.date == tomorrow)].dropna(subset=["CUSTOMER DELIVERY DATE"])
    
    if filtered_df.empty: return []

    # Grouping by Customer
    group_cols = ["CUSTOMER NAME", "CONTACT NUMBER", "SALES REP", "CUSTOMER DELIVERY DATE"]
    ...
    if alert_type == "payment":
        grouped_df["PENDING AMOUNT"] = grouped_df["ORDER AMOUNT"] - grouped_df["ADVANCE RECEIVED"]
